...
import { ShieldCheck, Lock, Eye, EyeOff,
  Loader2, ArrowLeft, CheckCircle, Shield } from "lucide-react";
...
export default function VerifyCode() {
    ...
    // Recupera el email que ForgotPassword envio al navegar
    ...
    // Si no hay email, redirige a ForgotPassword
    useEffect(() => {
      if (!email) {
    ...
    function handleCodeChange(index, value) {
      if (value && !/^\d$/.test(value)) return; // Solo permite digitos

      const newCode = [...code];
      newCode[index] = value;
      setCode(newCode);

      // Mueve el foco al siguiente input si se escribio un digito
      if (value && index < 5) {
      ...
      // Cuando el usuario presiona backspace en una caja vacia
      ...
      // Cuando el usuario pega el codigo completo
      ...
      async function handleSubmit(e) {
        ...
        if (codigoCompleto.length !== 6)
          return setMessage({
            type: "error",
            text: "Debes ingresar el codigo completo de 6 digitos"
          });
        ...
        try {
          const response = await axios.post(
            "http://localhost:8081/api/Recuperar/cambiar-passwords",
            {
              correo: email,
          ...
          setMessage({
            type: "success",
            text: response.data.message || "Contrasena actualizada"
          });
      ...
      if (!email) return null; // Guard: no renderiza sin email
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-4 py-3 border border-gray-300
                                rounded-lg pr-12 text-gray-900"
                      required
                    />
                    <button type="button"
